    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="max-w-3xl mb-16">
          <div className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-sunset-pop mb-4">
          ...
          <p className="text-base text-sunset-muted leading-relaxed max-w-xl">
          ...
          {services.map((service, index) => {
            ...
            return (
              <div
                ...
                className={`border border-sunset-border rounded-xl overflow-hidden transition-all duration-300 ${
                  isOpen ? 'bg-sunset-surface shadow-lg shadow-sunset-pop/5' : 'hover:border-sunset-pop/50'
                }`}
              >
                <button
                  onClick={() => setActiveIndex(isOpen ? null : index)}
                  className="w-full py-6 px-8 flex items-center justify-between text-left focus-sunset"
                >
                  <div className="flex items-center gap-6 md:gap-10">
                    <span className="font-mono text-sm font-bold text-sunset-pop">{service.number}</span>
                    <div>
                      <h3 className="text-xl md:text-2xl font-bold tracking-tight text-sunset-text">{service.title}</h3>
                      <p className="text-sm text-sunset-muted mt-1 hidden sm:block">{service.subtitle}</p>
                    </div>
                  </div>
                  <div className={`w-10 h-10 rounded-full border border-sunset-border flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-sunset-pop border-sunset-pop rotate-45' : 'hover:bg-sunset-surface'}`}>
                    <Plus className="w-5 h-5 text-sunset-text" />
                  </div>
                </button>

                {isOpen && (
                  <div className="px-8 pb-8 animate-fade-in">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 border-t border-sunset-border/50">
                      <div className="lg:col-span-7">
                        <p className="text-base text-sunset-muted leading-relaxed mb-6">
                          {service.description}
                        </p>
                        <button
                          onClick={scrollToContact}
                          className="inline-flex items-center gap-2 bg-sunset-pop text-sunset-bg px-6 py-3 rounded-full font-semibold text-sm transition-all hover:bg-sunset-pop/90 focus-sunset"
                        >
                      ...
                      <div className="lg:col-span-5">
                        <h4 className="text-xs font-mono uppercase tracking-wider text-sunset-muted/70 mb-4">
                          Deliverables
                        </h4>
                        <ul className="space-y-3">
                          {service.deliverables.map((item) => (
                            <li key={item} className="flex items-center gap-3 text-sm text-sunset-muted">
                              <span className="w-1.5 h-1.5 rounded-full bg-sunset-pop flex-shrink-0" />
                              {item}
                            </li>
                          ))}
                        </ul>
